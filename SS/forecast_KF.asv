function [output] = forecast_KF(y, ttm, model_options)
...
for r = 1:n_forecast
    ...
    optim_param_prior.(name) = par_optim;
    if LT == "GBM"
        model_par = 7;
    elseif LT == "OU"
        model_par = 8;
    ...
    if serial == "yes"
        [par_optim_temp, log_L_optim, par_init, trend, season, att, ytt, ett, vtt, fitresult_linear, fitresult_season, att_1, Ptt, Ptt_1] = ...
            param_estim_arp(y_temp(1:n_temp+r-1, :), ttm(1:n_temp+r-1, :), deltat, detrend_price, n_par_temp, par_names_temp, LT, correlation, par_optim_temp);
    elseif serial == "no"
        vtt = [];
    end
    optim_param.(name) = par_optim_temp;
    % optim_param{r}= par_optim_temp;
    
    ett_save.(name) = ett; att_save.(name) = att; ytt_save.(name) = ytt;
    vtt_save.(name) = vtt; lin_model.(name) = fitresult_linear; seas_model.(name) = fitresult_season;
    ...
    elseif serial == "yes"
        ...
        y_test(r,:) = d_test(:,end) + B_comb * x_test_temp;
        ytt_test.(name) = y_test;
        y_fore(r,:) = d_test(:,end) + B_comb * x_test_temp + fitresult_linear(n_temp+r) + fitresult_season(n_temp+r);
        ytt_fore.(name) = y_fore;
        y_temp = [y_temp; y_fore(r,:)];
        ytt_temp.(name) = y_temp;
        varn(:,:,r) = B_comb * (G_test * Ptt(:,:,end) * G_test' + W) * B_comb' + ...
            phiBGinv*W*phiBGinv'+ V_temp;
        
    end
end

output = struct('ytt_temp', ytt_temp, 'ytt_fore', ytt_fore, 'optim_param_prior', optim_param_prior, 'optim_param', optim_param,...
...
% output = struct('ytt_temp', ytt_temp, 'ytt_fore', ytt_fore, 'optim_param', optim_param,...
%     'ett_save', ett_save, 'ytt_save', ytt_save, 'vtt_save', vtt_save, 'att_save', att_save, 'lin_model', lin_model, ...
%     'seas_model', seas_model, 'a0_save', a0_save, 'x_test', x_test, 'Ptt_save', Ptt_save, 'varn', varn);